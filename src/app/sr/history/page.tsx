/**
 * Istorija Analiza - Pojednostavljena verzija
 * 
 * Prikazuje prethodne pikove sa jednostavnim indikatorima uspeha/neuspeha.
 * Čist, minimalan dizajn - samo bitne informacije.
 */
...
function getPickDisplay(analysis: AnalysisSummary): { pick: string; team: string | null } {
  ...
  if (!pick || pick === 'NONE') return { pick: 'Bez pika', team: null };
  ...
  if (pick === 'DRAW' || pick === 'draw') {
    return { pick: 'Nerešeno', team: null };
  }
...
function getOutcomeStatus(analysis: AnalysisSummary): { 
  ...
  if (outcome === 'HIT') {
    return { status: 'hit', label: '✓ Pogodak', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' };
  }
  if (outcome === 'MISS') {
    return { status: 'miss', label: '✗ Promašaj', color: 'text-red-400 bg-red-500/10 border-red-500/20' };
  }
  if (outcome === 'VOID' || outcome === 'PUSH') {
    return { status: 'void', label: '○ Poništeno', color: 'text-zinc-400 bg-zinc-500/10 border-zinc-500/20' };
  }
  if (outcome === 'PENDING') {
    return { status: 'pending', label: '⏳ Čekanje', color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' };
  }
  ...
  return { status: 'none', label: 'Predstojeći', color: 'text-zinc-500 bg-zinc-500/10 border-zinc-500/20' };
}
  ...
  return (
    <div className="min-h-screen bg-bg py-8 sm:py-12">
      <div className="max-w-2xl mx-auto px-4">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-white mb-1">Istorija Pikova</h1>
          <p className="text-zinc-500 text-sm">Evidencija tvojih analiziranih utakmica</p>
        </div>
        ...
        {history && history.analyses.length > 0 && (
          <div className="grid grid-cols-4 gap-3 mb-8">
            <div className="bg-white/[0.03] rounded-xl p-3 text-center border border-white/5">
              ...
              <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Ukupno</div>
            </div>
            <div className="bg-emerald-500/10 rounded-xl p-3 text-center border border-emerald-500/20">
              <div className="text-2xl font-bold text-emerald-400">{stats.hits}</div>
              <div className="text-[10px] text-emerald-400/70 uppercase tracking-wider">Pogoci</div>
            </div>
            <div className="bg-red-500/10 rounded-xl p-3 text-center border border-red-500/20">
              <div className="text-2xl font-bold text-red-400">{stats.misses}</div>
              <div className="text-[10px] text-red-400/70 uppercase tracking-wider">Promašaji</div>
            </div>
            <div className="bg-white/[0.03] rounded-xl p-3 text-center border border-white/5">
              <div className="text-2xl font-bold text-white">{hitRate}%</div>
              <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Uspešnost</div>
            </div>
          </div>
        )}
        ...
        {(!history || history.analyses.length === 0) && (
          <div className="text-center py-16">
            ...
            <h2 className="text-lg font-medium text-white mb-2">Nema pikova</h2>
            <p className="text-zinc-500 mb-6 text-sm">Počni analizirati utakmice da pratiš svoju evidenciju</p>
            <Link 
              href="/sr/matches" 
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-zinc-500">
                        {new Date(analysis.createdAt).toLocaleDateString('sr-Latn', { day: 'numeric', month: 'short' })}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    <div className="text-xs text-zinc-500 mb-0.5">Pik</div>
                  ...
                  {analysis.predictionOutcome?.actualScore && (
                    <div className="text-right">
                      <div className="text-xs text-zinc-500 mb-0.5">Rezultat</div>
        ...
        {history && history.analyses.length > 0 && (
          <div className="mt-8 text-center">
            ...
            >
              Pregledaj predstojeće utakmice →
            </Link>
          </div>
        )}
      </div>

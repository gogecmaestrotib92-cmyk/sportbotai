  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-bg-card/50 hover:bg-bg-hover border border-divider transition-colors"
        aria-label="Change language"
        aria-expanded={isOpen}
      >
        <span className="text-lg leading-none">{currentLanguage.flag}</span>
        {showLabel && <span className="text-sm text-text-secondary">{currentLanguage.name}</span>}
        <svg 

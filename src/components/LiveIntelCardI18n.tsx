/**
 * Live Intel Card Component - i18n version
 * 
...
const translations = {
  en: {
    ...
    open: 'Open →',
    noIntelPosts: 'No intel posts yet',
    readMore: 'Read more →',
    showLess: '← Show less',
    realTime: '⚡ Real-Time',
    follow: 'Follow',
  ...
  sr: {
    liveIntel: 'Uživo Info',
    autoUpdating: 'Auto-ažuriranje',
    open: 'Otvori →',
    noIntelPosts: 'Još nema objava',
    readMore: 'Pročitaj više →',
    showLess: '← Prikaži manje',
    realTime: '⚡ Uživo',
    follow: 'Prati',
    confidence: {
      LOW: 'Ograničeni Podaci',
      MEDIUM: 'Mešoviti Signali',
      HIGH: 'Jak Signal',
    },
  },
};
...
interface LiveIntelCardI18nProps {
  locale?: Locale;
}

export default function LiveIntelCardI18n({ locale = 'en' }: LiveIntelCardI18nProps) {
  ...
  const aiDeskLink = locale === 'sr' ? '/sr/ai-desk' : '/ai-desk';
  ...
  const formatTime = (timestamp: string) => {
    try {
      ...
      return locale === 'sr' ? 'nedavno' : 'recently';
    }
  };
      ...
      <div className="bg-[#0a0a0b] border border-emerald-500/30 rounded-xl sm:rounded-2xl overflow-hidden ring-1 ring-emerald-500/10">
        ...
        <div className="px-3 sm:px-5 py-3 sm:py-4 border-b border-white/[0.06] flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <span className="text-white font-semibold text-xs sm:text-sm block">{t.liveIntel}</span>
        ...
        <div className="divide-y divide-white/[0.04]">
          {loading ? (
            ...
            <div className="px-3 sm:px-5 py-6 sm:py-8 text-center">
              <p className="text-zinc-500 text-xs sm:text-sm">{t.noIntelPosts}</p>
            </div>
          ) : (
            // Real posts
            posts.map((post) => {
                ...
                <div className="flex items-start gap-2 sm:gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 mb-1 sm:mb-1.5">
                      ...
                      <span className={`px-1 sm:px-1.5 py-0.5 text-[8px] sm:text-[9px] font-semibold rounded border ${confidenceStylesMap[post.confidence]}`}>
                    ...
                    {isLongContent && (
                      ...
                      >
                        {isPostExpanded ? t.showLess : t.readMore}
                      </button>
                    )}

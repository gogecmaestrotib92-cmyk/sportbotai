...
export function parseMatchSlug(slug: string): {
  ...
  // These match the sport codes generated by getSportCode() from sport keys
  const knownSports = [
    // Basketball
    'nba', 'euroleague', 'ncaab',
    // American Football  
    'nfl', 'ncaaf',
    // Ice Hockey
    'nhl',
    // Soccer - short codes
    'epl', 'spl', 'mls',
    // Soccer - multi-word codes (country-league format)
    'spain-la-liga',
    'germany-bundesliga', 
    'italy-serie-a',
    'france-ligue-one',
  ...
  // Try to find a known sport from the end (up to 4 segments for "uefa-europa-conference-league")
  for (let i = 1; i <= Math.min(4, segments.length); i++) {
    const potentialSport = segments.slice(-i).join('-');
    if (knownSports.includes(potentialSport)) {
  ...
  // Fallback: if no known sport found, check if last segment is short (likely a sport code)
  if (!sportCode) {
    const lastSegment = segments[segments.length - 1];
    if (lastSegment.length <= 5) {
      sportCode = lastSegment;

...
export class BasketballAdapter extends BaseSportAdapter {
  ...
  /**
   * Get injuries for a basketball team using ESPN's free API
   */
  async getInjuries(teamId: string): Promise<DataLayerResponse<import('../types').NormalizedInjury[]>> {
    try {
      // teamId can be team name ("Denver Nuggets") or ID - ESPN provider handles both
      console.log(`[Basketball] Fetching injuries for team: ${teamId}`);

      const injuries = await getESPNInjuriesForTeam(teamId, 'nba');

      console.log(`[Basketball] Found ${injuries.length} injuries for ${teamId}`);

      return this.success(injuries);
    } catch (error) {
      console.error('[Basketball] Error fetching injuries:', error);
